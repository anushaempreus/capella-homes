...
const philosophy = [
  { tag: 'Mission', title: 'Building for the future', text: 'Built on principles and foundations from the past, newly inspired beautiful, healthy spaces and environments are an enduring legacy for the future.', image: 'https://images.unsplash.com/photo-1503387762-592deb58ef4e?w=800&q=80&fit=crop' },
  { tag: 'Vision',  title: 'Inspired by the past',   text: 'We exist to solve construction and development problems for people and create beautiful outcomes in the local Canberra area.',                    image: 'https://images.unsplash.com/photo-1600566752355-35792bedcfea?w=800&q=80&fit=crop' },
  { tag: 'Values',  title: 'Trust, quality, honesty', text: 'Everything we do is guided by trust, quality and honesty — values passed down through three generations of building excellence.',              image: 'https://images.unsplash.com/photo-1512917774080-9991f1c4c750?w=800&q=80&fit=crop' },
]
...
function TestimonialsSlider({ testimonials }: { testimonials: { quote: string; name: string }[] }) {
  ...
  return (
    <section style={{ background: '#f9f7f4', padding: '96px 80px', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', pointerEvents: 'none', zIndex: 0 }}>
        {[320, 480, 640].map((r, i) => (
          <div key={i} style={{ position: 'absolute', width: r, height: r, borderRadius: '50%', border: '1px solid rgba(28,33,48,0.04)', top: -r/2, left: -r/2 }}/>
        ))}
      </div>
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 900, margin: '0 auto' }}>
        <Reveal direction="up">
          ...
          <h2 style={{ fontFamily: SERIF, fontSize: 'clamp(28px, 3vw, 44px)', fontWeight: 700, color: '#1c2130', textAlign: 'center', margin: '0 0 56px', letterSpacing: '-0.5px' }}>What people say</h2>
        </Reveal>
        <Reveal direction="up" delay={0.15}>
          <div style={{ background: '#fff', borderRadius: 20, padding: '52px 56px', border: '1px solid rgba(28,33,48,0.07)', position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: 24, left: 40, fontFamily: SERIF, fontSize: 120, color: 'rgba(77,217,192,0.12)', lineHeight: 1, userSelect: 'none' }}>"</div>
            <div style={{ position: 'relative', zIndex: 1, minHeight: 160, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <p style={{
                fontFamily: SERIF, fontSize: 'clamp(18px, 2vw, 24px)', fontWeight: 400, color: '#1c2130', lineHeight: 1.7, margin: '0 0 32px', fontStyle: 'italic',
                opacity: visible ? 1 : 0,
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, opacity: visible ? 1 : 0, transition: 'opacity 0.4s ease 0.1s' }}>
                  <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(77,217,192,0.15)', display: 'grid', placeItems: 'center', border: '1px solid rgba(77,217,192,0.3)' }}>
                    <span style={{ fontFamily: SERIF, fontSize: 16, fontWeight: 700, color: '#4dd9c0' }}>{t.name[0]}</span>
                  </div>
                  <div style={{ fontFamily: SANS, fontSize: 13, fontWeight: 600, color: '#1c2130' }}>{t.name}</div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    {testimonials.map((_, i) => (
                      <div key={i} onClick={() => { setAnimDir(i > active ? 'right' : 'left'); setVisible(false); setTimeout(() => { setActive(i); setVisible(true) }, 300) }} style={{ height: 6, width: i === active ? 24 : 6, borderRadius: 3, background: i === active ? '#4dd9c0' : 'rgba(28,33,48,0.15)', transition: 'all 0.3s', cursor: 'pointer' }}/>
                    ))}
                  </div>
                  <button onClick={() => go('left')} style={{ width: 38, height: 38, borderRadius: '50%', border: '1px solid rgba(28,33,48,0.15)', background: 'transparent', cursor: 'pointer', display: 'grid', placeItems: 'center' }}>
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="#1c2130" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M11 7H3M6 10L3 7l3-3"/></svg>
                  </button>
                  <button onClick={() => go('right')} style={{ width: 38, height: 38, borderRadius: '50%', border: 'none', background: '#4dd9c0', cursor: 'pointer', display: 'grid', placeItems: 'center' }}>
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="#050810" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 7h8M8 4l3 3-3 3"/></svg>
                  </button>
                </div>
              </div>
            </div>
  ...
  return (
    <main style={{ fontFamily: SANS }}>
      ...
      <section style={{ position: 'relative', height: '90vh', minHeight: 600, overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, backgroundImage: 'url(https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?w=1800&q=80&fit=crop)', backgroundSize: 'cover', backgroundPosition: 'center' }}/>
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(4,7,18,0.85) 0%, rgba(4,7,18,0.3) 60%, rgba(4,7,18,0.1) 100%)' }}/>
        ...
        <div style={{ position: 'absolute', bottom: 60, left: 0, right: 0, zIndex: 3, padding: '0 80px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
              <a href="/" style={{ fontFamily: SANS, fontSize: 11, color: 'rgba(255,255,255,0.5)', textDecoration: 'none', textTransform: 'uppercase', letterSpacing: '1.5px' }}>Home</a>
      ...
      <section style={{ background: '#ffffff' }}>
        <div style={{ height: 3, background: 'linear-gradient(to right, #4dd9c0 30%, rgba(77,217,192,0.1))' }}/>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: 580 }}>

          {/* Left — clipped image */}
          <Reveal direction="left">
            <div style={{ position: 'relative', background: '#f9f7f4', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '60px 40px', height: '100%' }}>
              <div style={{ position: 'relative', width: '100%', maxWidth: 420, aspectRatio: '4/5' }}>
                <div style={{ position: 'absolute', inset: 0, backgroundImage: 'url(https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=800&q=80&fit=crop)', backgroundSize: 'cover', backgroundPosition: 'center', clipPath: 'polygon(0 0, 100% 4%, 100% 96%, 0 100%)' }}/>
                <div style={{ position: 'absolute', inset: 0, border: '2px solid #4dd9c0', clipPath: 'polygon(0 0, 100% 4%, 100% 96%, 0 100%)', pointerEvents: 'none', opacity: 0.4 }}/>
              </div>
              <div style={{ position: 'absolute', top: 32, left: 32 }}>
                <svg viewBox="0 0 120 120" width="90" height="90">
                  ...
                  <circle cx="60" cy="60" r="42" fill="rgba(10,14,26,0.85)" stroke="rgba(77,217,192,0.3)" strokeWidth="1"/>
                  <polygon points="60,45 70,60 50,60" fill="#4dd9c0" opacity="0.9"/>
                  <text
                    fill="rgba(255,255,255,0.6)"
                    style={{ fontSize: '7.5px', fontFamily: 'DM Sans, sans-serif', fontWeight: 600, letterSpacing: '2.5px' }}
                  >
          ...
          <Reveal direction="right" delay={0.1}>
            <div style={{ padding: '80px 72px 80px 56px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 24 }}>
              ...
              <div>
                <h2 style={{ fontFamily: SERIF, fontSize: 'clamp(28px, 3vw, 46px)', fontWeight: 400, color: 'rgba(28,33,48,0.5)', margin: 0, lineHeight: 1.1, letterSpacing: '-0.5px' }}>From Your Dreams</h2>
                <h2 style={{ fontFamily: SERIF, fontSize: 'clamp(28px, 3vw, 46px)', fontWeight: 700, color: '#1c2130', margin: 0, lineHeight: 1.1, letterSpacing: '-0.5px' }}>to Your Reality</h2>
              </div>
              ...
              <a href="/projects" style={{ fontFamily: SANS, display: 'inline-flex', alignItems: 'center', gap: 10, background: '#1c2130', color: '#fff', padding: '13px 28px', borderRadius: 8, fontWeight: 600, fontSize: 11, letterSpacing: '1.5px', textTransform: 'uppercase', textDecoration: 'none', width: 'fit-content' }}>
                View Projects
      ...
      {/* ── PHILOSOPHY ── */}
      <section style={{ background: '#0a0e1a', padding: '96px 80px', position: 'relative', overflow: 'hidden' }}>
            ...
            {philosophy.map((p, i) => (
              ...
              >
                <div style={{ height: 220, position: 'relative', overflow: 'hidden' }}>
                  <div className="phil-img" style={{ position: 'absolute', inset: 0, backgroundImage: `url(${p.image})`, backgroundSize: 'cover', backgroundPosition: 'center', transition: 'transform 0.7s ease', opacity: 0.07 }}/>
                  <div style={{ position: 'absolute', inset: 0, background: '#0d1520' }}/>
                  <div className="phil-line" style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: '#4dd9c0', width: '30%', transition: 'width 0.4s ease' }}/>
                  <div style={{ position: 'absolute', inset: 0, padding: '32px 32px', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                    <div style={{ fontFamily: SANS, fontSize: 10, fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: '#4dd9c0', marginBottom: 10 }}>{p.tag}</div>
